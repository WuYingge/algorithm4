 ...
 */
public final class StdDraw3D implements MouseListener, MouseMotionListener, MouseWheelListener, KeyListener,
        ActionListener, ChangeListener, ComponentListener, WindowFocusListener {
    ...
    private static JRadioButtonMenuItem orbitModeButton, fpsModeButton, airplaneModeButton, lookModeButton,
            fixedModeButton;
    ...
    private static final BoundingSphere INFINITE_BOUNDS = new BoundingSphere(new Point3d(0.0, 0.0, 0.0), 1e100);
    ...
    private static JMenuBar createMenuBar() {
        ...
        orbitModeButton.setAccelerator(
        ...
        airplaneModeButton.setAccelerator(
        ...
        fixedModeButton.setAccelerator(
        ...
        SpinnerNumberModel snm2 = new SpinnerNumberModel(DEFAULT_NUM_DIVISIONS, 4, 4000, 5);
    ...
    // FIX THIS check that adding specular didn't mess up custom shapes (lines,
    // triangles, points)
    ...
    /**
     * Scales the given x-coordinate from user coordinates into 2D pixel
     * coordinates.
     */
    ...
    /**
     * Scales the given y-coordinate from user coordinates into 2D pixel
     * coordinates.
     */
    ...
    /**
     * Scales the given x-coordinate from 2D pixel coordinates into user
     * coordinates.
     */
    ...
    /**
     * Scales the given y-coordinate from 2D pixel coordinates into user
     * coordinates.
     */
     ...
     */
    public static void setPenRadius(double w) {
        penRadius = (float) w;
    }
    ...
    /**
     * Sets the font to draw text with. The font name and point size are given as
     * parameters.
     * 
    ...
    /**
     * Sets the font to draw text with. The font name, style, and size are given as
     * parameters. The style can be a bitwise combination of StdDraw3D.PLAIN,
     * StdDraw3D.ITALIC, and StdDraw3D.BOLD.
     * 
     * @param name  The name of the font.
    ...
    /**
     * Sets anti-aliasing on or off. Anti-aliasing makes graphics look much
     * smoother, but is very resource heavy. It is good for saving images that look
     * professional. The default is off.
     * 
    ...
    /**
     * Sets the number of triangular divisons of curved objects. The default is 100
     * divisons. Decrease this to increase performance.
     * 
    ...
    /**
     * Sets the projection mode to perspective projection with a default field of
     * view. In this mode, closer objects appear larger, as in real life.
     */
    ...
    /**
     * Sets the projection mode to perspective projection with the given field of
     * view. In this mode, closer objects appear larger, as in real life. A larger
     * field of view means that there is more perspective distortion. Reasonable
     * values are between 0.5 and 3.0.
     * 
    ...
    /**
     * Sets the projection mode to orthographic projection. In this mode, parallel
     * lines remain parallel after projection, and there is no perspective. It is as
     * looking from infinitely far away with a telescope. AutoCAD programs use this
     * projection mode.
     */
    ...
    private static void mouseMotionEvents(MouseEvent e, double newX, double newY, boolean dragged) {
        ...
        if ((cameraMode == ORBIT_MODE) && (dragged && isKeyPressed(KeyEvent.VK_ALT))
    ...
    /**
     * Is the given key currently pressed down? The keys correnspond to physical
     * keys, not characters. For letters, use the uppercase character to refer to
     * the key. For arrow keys and modifiers such as shift and ctrl, refer to the
     * KeyEvent constants, such as KeyEvent.VK_SHIFT.
     * 
    ...
    /**
     * Sets the background to the given image file. The file gets wrapped around as
     * a spherical skybox.
     * 
    ...
    // public static void playPointSound (String filename, Vector3D position, double
    // volume, boolean loop) {
    //
    // MediaContainer mc = new MediaContainer("file:" + filename);
    // mc.setCacheEnable(true);
    ...
    /**
     * Adds a directional light of color col which shines in the direction vector
     * (x, y, z)
     */
     ...
     * drawing a complicated scene once and then exploring without redrawing. Call
     * only as the last line of a program.
     */
    ...
    /**
     * Renders drawn 3D objects to the screen and draws the 2D overlay, then pauses
     * for the given time.
     * 
     * @param time The number of milliseconds to pause for.
     */
    ...
    /**
     * Draws a sphere at (x, y, z) with radius r, axial rotations (xA, yA, zA), and
     * a texture from imageURL.
     */
    public static Shape sphere(double x, double y, double z, double r, double xA, double yA, double zA,
            String imageURL) {
    ...
    /**
     * Draws a wireframe sphere at (x, y, z) with radius r and axial rotations (xA,
     * yA, zA).
     */
    ...
    /**
     * Draws an ellipsoid at (x, y, z) with dimensions (w, h, d) and axial rotations
     * (xA, yA, zA).
     */
    public static Shape ellipsoid(double x, double y, double z, double w, double h, double d, double xA, double yA,
            double zA) {
        return ellipsoid(x, y, z, w, h, d, xA, yA, zA, null);
    }

    /**
     * Draws an ellipsoid at (x, y, z) with dimensions (w, h, d) and a texture from
     * imageURL.
     */
    ...
    /**
     * Draws an ellipsoid at (x, y, z) with dimensions (w, h, d), axial rotations
     * (xA, yA, zA), and a texture from imageURL.
     */
    public static Shape ellipsoid(double x, double y, double z, double w, double h, double d, double xA, double yA,
            double zA, String imageURL) {
    ...
    /**
     * Draws a wireframe ellipsoid at (x, y, z) with dimensions (w, h, d) and axial
     * rotations (xA, yA, zA).
     */
    public static Shape wireEllipsoid(double x, double y, double z, double w, double h, double d, double xA, double yA,
            double zA) {
    ...
    /**
     * Draws a cube at (x, y, z) with radius r, axial rotations (xA, yA, zA), and a
     * texture from imageURL.
     */
    ...
    /**
     * Draws a wireframe cube at (x, y, z) with radius r and axial rotations (xA,
     * yA, zA).
     */
    ...
    public static Shape wireCube(double x, double y, double z, double r) {
        ...
        double[] xC = new double[] { x + r, x + r, x - r, x - r, x + r, x + r, x + r, x - r, x - r, x + r, x + r, x + r,
                x - r, x - r, x - r, x - r };
        double[] yC = new double[] { y + r, y - r, y - r, y + r, y + r, y + r, y - r, y - r, y + r, y + r, y - r, y - r,
                y - r, y - r, y + r, y + r };
        double[] zC = new double[] { z + r, z + r, z + r, z + r, z + r, z - r, z - r, z - r, z - r, z - r, z - r, z + r,
                z + r, z - r, z - r, z + r };
    ...
    /**
     * Draws a box at (x, y, z) with dimensions (w, h, d) and axial rotations (xA,
     * yA, zA).
     */
    public static Shape box(double x, double y, double z, double w, double h, double d, double xA, double yA,
            double zA) {
        return box(x, y, z, w, h, d, xA, yA, zA, null);
    }

    /**
     * Draws a box at (x, y, z) with dimensions (w, h, d) and a texture from
     * imageURL.
     */
    ...
    /**
     * Draws a box at (x, y, z) with dimensions (w, h, d), axial rotations (xA, yA,
     * zA), and a texture from imageURL.
     */
    public static Shape box(double x, double y, double z, double w, double h, double d, double xA, double yA, double zA,
        ...
        com.sun.j3d.utils.geometry.Box box = new com.sun.j3d.utils.geometry.Box(dimensions.x, dimensions.y,
                dimensions.z, PRIMFLAGS, ap, numDivisions);
    ...
    /**
     * Draws a wireframe box at (x, y, z) with dimensions (w, h, d) and axial
     * rotations (xA, yA, zA).
     */
    public static Shape wireBox(double x, double y, double z, double w, double h, double d, double xA, double yA,
            double zA) {
    ...
    /**
     * Draws a cylinder at (x, y, z) with radius r, height h, and axial rotations
     * (xA, yA, zA).
     */
    ...
    /**
     * Draws a cylinder at (x, y, z) with radius r, height h, and a texture from
     * imageURL.
     */
    ...
    /**
     * Draws a cylinder at (x, y, z) with radius r, height h, axial rotations (xA,
     * yA, zA), and a texture from imageURL.
     */
    public static Shape cylinder(double x, double y, double z, double r, double h, double xA, double yA, double zA,
            String imageURL) {
    ...
    /**
     * Draws a wireframe cylinder at (x, y, z) with radius r, height h, and axial
     * rotations (xA, yA, zA).
     */
    public static Shape wireCylinder(double x, double y, double z, double r, double h, double xA, double yA,
            double zA) {
    ...
    /**
     * Draws a cone at (x, y, z) with radius r, height h, and axial rotations (xA,
     * yA, zA).
     */
    ...
    /**
     * Draws a cone at (x, y, z) with radius r, height h, and a texture from
     * imageURL.
     */
    ...
    /**
     * Draws a cone at (x, y, z) with radius r, height h, axial rotations (xA, yA,
     * zA), and a texture from imageURL.
     */
    public static Shape cone(double x, double y, double z, double r, double h, double xA, double yA, double zA,
            String imageURL) {
    ...
    /**
     * Draws a wireframe cone at (x, y, z) with radius r, height h, and axial
     * rotations (xA, yA, zA).
     */
    ...
    /**
     * Draws a Java 3D Primitive object at (x, y, z) with axial rotations (xA, yA,
     * zA).
     */
    ...
    /**
     * Draws a set of points at the given coordinates. For example, the first point
     * is at (x[0], y[0], z[0]). Much more efficient than drawing individual points.
     */
    ...
    /**
     * Draws a set of points at the given coordinates with the given colors. For
     * example, the first point is at (x[0], y[0], z[0]) with color colors[0]. Much
     * more efficient than drawing individual points.
     */
    ...
    /**
     * Draws a set of connected lines. For example, the first line is from (x[0],
     * y[0], z[0]) to (x[1], y[1], z[1]), and the second line is from (x[1], y[1],
     * z[1]) to (x[2], y[2], z[2]). Much more efficient than drawing individual
     * lines.
     */
    public static Shape lines(double[] x, double[] y, double[] z) {
        ...
        GeometryArray geom = new LineStripArray(coords.length, LineArray.COORDINATES, new int[] { coords.length });
        geom.setCoordinates(0, coords);
     ...
     * y[0], z[0]) to (x[1], y[1], z[1]), and the second line is from (x[1], y[1],
     * z[1]) to (x[2], y[2], z[2]). Much more efficient than drawing individual
     * lines. Vertex colors are specified by the given array, and line colors are
     * blends of its two vertex colors.
     */
    public static Shape lines(double[] x, double[] y, double[] z, Color[] colors) {

        Point3f[] coords = constructPoint3f(x, y, z);

        GeometryArray geom = new LineStripArray(coords.length, LineArray.COORDINATES | LineArray.COLOR_4,
    ...
    /**
     * Draws a cylindrical tube of radius r from vertex (x1, y1, z1) to vertex (x2,
     * y2, z2).
     */
    ...
    /**
     * Draws a series of cylindrical tubes of radius r, with vertices (x, y, z) and
     * the given colors. No more efficient than drawing individual tubes.
     */
    ...
    /**
     * Draws a filled polygon with the given vertices. The vertices should be planar
     * for a proper 2D polygon to be drawn.
     */
    ...
    /**
     * Draws a polygon with the given vertices, which is filled or outlined based on
     * the argument.
     * 
    ...
    /**
     * Draws triangles which are defined by x1=points[i][0], y1=points[i][1],
     * z1=points[i][2], x2=points[i][3], etc. All of the points will be the width
     * and color specified by the setPenColor and setPenWidth methods.
     * 
     * @param points an array of the points to be connected. The first dimension is
     *               unspecified, but the second dimension should be 9 (the 3-space
    ...
    /**
     * Draws a set of wireframetriangles, each with the specified color. There
     * should be one color for each triangle. This is much more efficient than
     * drawing individual triangles.
     */
    ...
    /**
     * Draws a 3D text object at (x, y, z). Uses the pen font, color, and
     * transparency.
     */
    ...
    public static void overlayPoint(double x, double y) {
        ...
        if (r <= 1)
            ...
            getGraphics2D(offscreenImage).fill(new Ellipse2D.Double(scaleX(x) - r / 2, scaleY(y) - r / 2, r, r));
    }
    ...
    public static void overlayCircle(double x, double y, double r) {
        ...
        if (ws <= 1 && hs <= 1)
            ...
            getGraphics2D(offscreenImage).draw(new Ellipse2D.Double(xs - ws / 2, ys - hs / 2, ws, hs));
    }
    ...
    public static void overlayFilledCircle(double x, double y, double r) {
        ...
        if (ws <= 1 && hs <= 1)
            ...
            getGraphics2D(offscreenImage).fill(new Ellipse2D.Double(xs - ws / 2, ys - hs / 2, ws, hs));
    }
    ...
    /**
     * Draws a filled ellipse with given semimajor and semiminor axes, centered on
     * (x, y).
     */
    ...
    public static void overlaySquare(double x, double y, double r) {
        ...
        if (ws <= 1 && hs <= 1)
            ...
            getGraphics2D(offscreenImage).draw(new Rectangle2D.Double(xs - ws / 2, ys - hs / 2, ws, hs));
    }
    ...
    public static void overlayFilledSquare(double x, double y, double r) {
        ...
        if (ws <= 1 && hs <= 1)
            ...
            getGraphics2D(offscreenImage).fill(new Rectangle2D.Double(xs - ws / 2, ys - hs / 2, ws, hs));
    }
    ...
    /**
     * Draws the given text as stationary on the window at (x, y). This is useful
     * for titles and HUD-style text.
     */
    ...
    public static void overlayPicture(double x, double y, String s) {
        ...
        getGraphics2D(offscreenImage).drawImage(image, (int) Math.round(xs - ws / 2.0), (int) Math.round(ys - hs / 2.0),
                null);
    }

    /**
     * Draws a picture (gif, jpg, or png) centered on (x, y), rotated given number
     * of degrees.
     */
    ...
    public static void overlayPicture(double x, double y, String s, double w, double h) {
        ...
        if (ws <= 1 && hs <= 1)
            ...
            getGraphics2D(offscreenImage).drawImage(image, (int) Math.round(xs - ws / 2.0),
                    (int) Math.round(ys - hs / 2.0), (int) Math.round(ws), (int) Math.round(hs), null);
        }
    }
    ...
    public static void overlayPicture(double x, double y, String s, double w, double h, double degrees) {
        ...
        Graphics2D graphics = getGraphics2D(offscreenImage);
        graphics.rotate(Math.toRadians(-degrees), xs, ys);
        graphics.drawImage(image, (int) Math.round(xs - ws / 2.0), (int) Math.round(ys - hs / 2.0),
    ...
    private static void infoDisplay() {
        ...
        g.setStroke(new BasicStroke(DEFAULT_PEN_RADIUS, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
        ...
        // String s1 = "(" + df.format(forward.x) + "," + df.format(forward.y) + "," +
        // df.format(forward.z) + ")";
    ...
    public static void save(String filename) {
        ...
        javax.media.j3d.Raster ras = new javax.media.j3d.Raster(new Point3f(-1.0f, -1.0f, -1.0f),
                javax.media.j3d.Raster.RASTER_COLOR, 0, 0, width, height, imageComp, null);
    ...
    public static void saveScene3D(String filename) {
        ...
        File file = new File(filename);
        // System.out.println("on: " + onscreenGroup.numChildren() + " off: " +
        // offscreenGroup.numChildren());

        try {
            SceneGraphFileWriter writer = new SceneGraphFileWriter(file, universe, false,
                    "3D scene saved from StdDraw3D.", null);
